...
def upload_item_with_retry(doc, container, log_area, MAX_RETRIES=5):
...
def upload_sku_price(raw_sku, raw_pc, raw_price, log_area):
    ...
    log_area.insert(tk.END, f"Attempting upload for ID: {doc['id']}...\n")
    log_area.see(tk.END)
    success, ru, error_msg = upload_item_with_retry(doc, container, log_area, MAX_RETRIES=5)
    if success:
...
def bulk_upload(file_path, log_area):
    ...
    try:
        ...
        log_area.insert(tk.END, f"Starting concurrent upload using {max_workers} threads for {len(valid_docs)} documents...\n")
        log_area.see(tk.END)
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_doc = {
                executor.submit(upload_item_with_retry, doc, container, log_area, MAX_RETRIES): doc
...
def apply_settings_from_ui(conn_var, db_var, cont_var, partial_var, loglevel_var, max_workers_var):
...
def build_app():
    ...
    apply_btn = ttk.Button(btn_frame, text="Apply (No Save)", 
                           command=lambda: (apply_settings_from_ui(conn_var, db_var, cont_var, partial_var, loglevel_var, max_workers_var), messagebox.showinfo("Settings", "Settings applied.")))
    apply_btn.grid(row=0, column=0, padx=4)
    save_btn = ttk.Button(btn_frame, text="Save Settings", 
                          command=lambda: (apply_settings_from_ui(conn_var, db_var, cont_var, partial_var, loglevel_var, max_workers_var), save_settings()))
